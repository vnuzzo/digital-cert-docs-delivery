...
@Slf4j
@Component
public class PecProviderAdapter implements DeliveryProviderPort {
    ...
    public Mono<ProviderResponse> send(String requestId, DeliveryType deliveryType, List<String> recipients, List<String> documents) {
        ...
        log.info("Entered PecProviderAdapter.send for [requestId: {} - deliveryType: {} - recipients: {} - documents: {}]", requestId,
                deliveryType, recipients, documents);

        return Mono.fromSupplier(() -> {

            log.info("PEC SEND (stub) requestId: {}", requestId);

            return new ProviderResponse("PEC-" + UUID.randomUUID());
